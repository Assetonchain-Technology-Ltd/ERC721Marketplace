const expensetxhash = require('../model/expensetxhash.model');
...
exports.insert = function(req,res){
	if(req.body.batch){
		...
		expensetxhash.insertMany(req.body.batch,function(err,docs){
			if(err){
				logger.error(`ExpenseTxHash insert Error - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.status(500).send(err);
			}
			else{
				logger.info(`ExpenseTxHash insert - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.json(req.body);
			}
		});		
	}
	else{
		logger.error(`ExpenseTxHash insert - No Batch detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
...
exports.updateExpenseTxHash = function(req,res){
	if(req.body.key && req.body.value){
		//Use insertMany to validate the doc before save, all or nothing
		expensetxhash.updateOne(req.body.key,req.body.value,function(err,docs){
				if(err){
					logger.error(`ExpenseTxHash update ${JSON.stringify(req.body.key)} Fail - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.status(500).send(err);
				}
				else{
					logger.info(`ExpenseTxHash update ${JSON.stringify(req.body.key)} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.json(docs);
		...
		logger.error(`ExpenseTxHash update - No key/value paire detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
