const orderbook = require('../model/orderbook.model');
...
exports.insert = function(req,res){
	if(req.body.batch){
		...
		orderbook.insertMany(req.body.batch,function(err,docs){
			if(err){
				logger.error(`Orderbook insert Error - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.status(500).send(err);
			}
			else{
				logger.info(`Orderbook insert - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.json(req.body);
			}
		});		
	}
	else{
		logger.error(`Orderbook insert - No Batch detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
...
exports.listAllTrade = function(req,res){
			orderbook.find({},null,{sort: {OPEN_createdate:-1}},function(err,docs){
					if(err){
						logger.error(`Orderbook getAllTrade() - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
						res.status(500).send(err);
					}else{
						logger.info(`Orderbook getAllTrade() - ${req.originalUrl} - ${req.method} - ${req.ip}`);
						res.send(docs);
					}
					});
};
...
exports.updateOrderbook = function(req,res){
	if(req.body.key && req.body.value){
		//Use insertMany to validate the doc before save, all or nothing
		orderbook.updateOne(req.body.key,req.body.value,function(err,docs){
				if(err){
					logger.error(`Orderbook update ${JSON.stringify(req.body.key)} Fail - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.status(500).send(err);
				}
				else{
					logger.info(`Orderbook update ${JSON.stringify(req.body.key)} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.json(docs);
		...
		logger.error(`Orderbook update - No key/value paire detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
