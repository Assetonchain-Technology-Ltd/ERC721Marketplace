const ordertxhash = require('../model/ordertxhash.model');
...
exports.insert = function(req,res){
	if(req.body.batch){
		...
		ordertxhash.insertMany(req.body.batch,function(err,docs){
			if(err){
				logger.error(`OrderTxHash insert Error - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.status(500).send(err);
			}
			else{
				logger.info(`OrderTxHash insert - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.json(req.body);
			}
		});		
	}
	else{
		logger.error(`OrderTxHash insert - No Batch detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
...
exports.updateOrderTxHash = function(req,res){
	if(req.body.key && req.body.value){
		//Use insertMany to validate the doc before save, all or nothing
		ordertxhash.updateOne(req.body.key,req.body.value,function(err,docs){
				if(err){
					logger.error(`OrderTxHash update ${JSON.stringify(req.body.key)} Fail - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.status(500).send(err);
				}
				else{
					logger.info(`OrderTxHash update ${JSON.stringify(req.body.key)} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.json(docs);
		...
		logger.error(`OrderTxHash update - No key/value paire detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
