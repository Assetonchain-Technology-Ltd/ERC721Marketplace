const paymenttxhash = require('../model/paymenttxhash.model');
...
exports.insert = function(req,res){
	if(req.body.batch){
		...
		paymenttxhash.insertMany(req.body.batch,function(err,docs){
			if(err){
				logger.error(`PaymentTxhash insert Error - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.status(500).send(err);
			}
			else{
				logger.info(`PaymentTxhash insert - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.json(req.body);
			}
		});		
	}
	else{
		logger.error(`PaymentTxhash insert - No Batch detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
...
exports.updatePaymentTxhash = function(req,res){
	if(req.body.key && req.body.value){
		//Use insertMany to validate the doc before save, all or nothing
		paymenttxhash.updateOne(req.body.key,req.body.value,function(err,docs){
				if(err){
					logger.error(`PaymentTxhash update ${JSON.stringify(req.body.key)} Fail - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.status(500).send(err);
				}
				else{
					logger.info(`PaymentTxhash update ${JSON.stringify(req.body.key)} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.json(docs);
		...
		logger.error(`PaymentTxhash update - No key/value paire detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
