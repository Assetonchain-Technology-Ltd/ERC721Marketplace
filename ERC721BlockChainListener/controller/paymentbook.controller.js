const paymentbook = require('../model/paymentbook.model');
...
exports.insert = function(req,res){
	if(req.body.batch){
		...
		paymentbook.insertMany(req.body.batch,function(err,docs){
			if(err){
				logger.error(`Paymentbook insert Error - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.status(500).send(err);
			}
			else{
				logger.info(`Paymentbook insert - ${req.originalUrl} - ${req.method} - ${req.ip}`);
				res.json(req.body);
			}
		});		
	}
	else{
		logger.error(`Paymentbook insert - No Batch detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
...
exports.updatePaymentbook = function(req,res){
	if(req.body.key && req.body.value){
		//Use insertMany to validate the doc before save, all or nothing
		paymentbook.updateOne(req.body.key,req.body.value,function(err,docs){
				if(err){
					logger.error(`Paymentbook update ${JSON.stringify(req.body.key)} Fail - ${err.message} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.status(500).send(err);
				}
				else{
					logger.info(`Paymentbook update ${JSON.stringify(req.body.key)} - ${req.originalUrl} - ${req.method} - ${req.ip}`);
					res.json(docs);
		...
		logger.error(`Paymentbook update - No key/value paire detected - ${req.originalUrl} - ${req.method} - ${req.ip}`);
